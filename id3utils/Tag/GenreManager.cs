...
namespace ID3Utils
{
	...
	public class StandardGenreManager
		: IGenreManager
	{
		private static Dictionary<string, byte> genreDictionary=null;
		...
		private static void LoadGenereDictionary()
		{
			...
			genreDictionary=new Dictionary<string, byte>();
		...
		public byte GetGenreCode(string genreName)
		{
			...
			if(genreDictionary.ContainsKey(genreName.ToLower()))
			{
				return genreDictionary[genreName.ToLower()];
			}
		...
		{
			if(genreDictionary.ContainsKey(genre.ToLower()))
